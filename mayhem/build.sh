...
# --- NDEBUG: the reason both targets recorded 0 edges -------------------------------------
# PoDoFo's internal PODOFO_ASSERT(x) expands to a plain assert(x) whenever NDEBUG is
# undefined (src/podofo/private/PdfDeclarationsPrivate.h). Several of those assertions
# encode invariants that only hold for a WELL-FORMED file, so on a fuzz corpus they are not
# "something went wrong" -- they ARE the normal rejection path. The fatal one is the first
# statement of PdfParser::tryRebuildCrossReference(), which asserts that a PDF version was
# already detected; PdfParser::Parse() reaches it from its catch block for ANY input without
# a valid "%PDF-x.y" header -- i.e. essentially every byte string libFuzzer generates:
#
#   fuzz_load: src/podofo/private/PdfParser.cpp:235: bool
#   PoDoFo::PdfParser::tryRebuildCrossReference(InputStreamDevice &):
#   Assertion `m_PdfVersion != PdfVersion::Unknown' failed.
#
# assert() abort()s, and an abort is not a C++ exception, so the harnesses' catch clauses
# never see it: the process died on libFuzzer's FIRST generated input (a single 0x0a byte),
# before it printed one iteration or reported one edge. That is why fuzz_load and
# fuzz_load_text both sat at 0 edges / 0 defects -- the harness was never fuzzing at all.
#
# CMake only defines NDEBUG through CMAKE_<LANG>_FLAGS_<CONFIG>, and step 1 below
# deliberately sets no CMAKE_BUILD_TYPE (it supplies the flags itself), so NDEBUG was never
# defined for the fuzz tree -- while every SHIPPED PoDoFo build defines it, including the
# Release build of the Catch2 oracle in step 2. Define it explicitly so the fuzzed library
# matches the configuration PoDoFo is actually released in, and a debug-only assertion stops
# masquerading as a crash on every input.
#
# This weakens NOTHING about the sanitizers: $SANITIZER_FLAGS (ASan+UBSan, halting via
# -fno-sanitize-recover=all) is untouched and still applied on top, so real memory-safety
# and undefined-behaviour bugs abort exactly as before -- they are now actually REACHABLE,
# because the fuzzer gets past input #1.
#
# -O1 rides along: with no CMAKE_BUILD_TYPE, CMake emits no -O at all, and an unoptimised
# ASan build of a PDF parser fuzzes about an order of magnitude slower. -O1 keeps inlining
# light so ASan/libFuzzer stack traces stay readable.
: "${RELEASE_FLAGS:=-DNDEBUG -O1}"
: "${CC:=clang}" ; : "${CXX:=clang++}" ; : "${LIB_FUZZING_ENGINE:=-fsanitize=fuzzer}"
: "${STANDALONE_FUZZ_MAIN:=/opt/mayhem/StandaloneFuzzTargetMain.c}"
: "${MAYHEM_JOBS:=$(nproc)}"
export SANITIZER_FLAGS DEBUG_FLAGS RELEASE_FLAGS CC CXX LIB_FUZZING_ENGINE STANDALONE_FUZZ_MAIN MAYHEM_JOBS
: "${SRC:=/mayhem}"
cd "$SRC"
...
cmake -S "$SRC" -B "$FUZZ_BUILD" "${CMAKE_COMMON[@]}" \
  ...
  -DCMAKE_C_FLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS $RELEASE_FLAGS" \
  -DCMAKE_CXX_FLAGS="$SANITIZER_FLAGS $DEBUG_FLAGS $RELEASE_FLAGS" \
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $RELEASE_FLAGS -c -x c "$STANDALONE_FUZZ_MAIN" -o "$BUILD_ROOT/standalone_main.o"

HARNESS_DIR="$SRC/mayhem/harnesses"
for h in fuzz_load fuzz_load_text; do
  $CXX $SANITIZER_FLAGS $DEBUG_FLAGS $RELEASE_FLAGS $PODOFO_CFLAGS \
